    ...
    public static class SRDebuggerUtil
    {
        ...
        public static ICollection<OptionDefinition> ScanForOptions(object obj)
        {
            ...
            foreach (var memberInfo in members)
            {
                ...
                if (nameAttribute != null && nameAttribute.UseProperty)
                {
                    var prop = obj.GetType().GetProperty(nameAttribute.Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    if (prop != null)
                    {
                        name = prop.GetValue(obj).ToString();
                    }
                }
