			...
			if (theDistanceSqr > _minVertexDistanceSqr)
			{
				...
				if (make)
				{
					...
					if (_points.Count >= 4)
					{
						...
						PooledList<Vector3> smoothTipList = new PooledList<Vector3>(smoothTip);
						PooledList<Vector3> smoothBaseList = new PooledList<Vector3>(smoothBase);
						...
						smoothTipList.Dispose();
						smoothBaseList.Dispose();
					}
#endif
				}
				else
